...
mutation_TNN_patientID = data_mutation_common$PATIENT_ID[data_mutation_common$Hugo_Symbol == "TNN"]

data_clinical_common_TNN = subset(data_clinical_common, PATIENT_ID %in% mutation_TNN_patientID)
...
SA on the most mutated gene sex
```{r}
Surv(data_clinical_common_TNN$overall_survival, data_clinical_common_TNN$deceased) ~ data_clinical_common_TNN$SEX

fit = survfit(Surv(overall_survival, deceased) ~ SEX, data = data_clinical_common_TNN)

table(data_clinical_common_TNN$SEX)
pval = surv_pvalue(fit, data=data_clinical_common_TNN)$pval
print(pval)

ggsurvplot(fit, data=data_clinical_common_TNN, pval=T, risk.table=T, risk.table.col="strata", risk.table.height=0.35)
```
SA on the most mutated gene tumor stage
```{r}
data_clinical_common_TNN$deceased = data_clinical_common_TNN$PFS_STATUS == "1:PROGRESSION"

# create an "overall survival" variable that is equal to days_to_death
# for dead patients, and to days_to_last_follow_up for patients who
# are still alive
data_clinical_common_TNN$overall_survival = ifelse(data_clinical_common_TNN$deceased,
                                   data_clinical_common_TNN$OS_MONTHS,
                                   data_clinical_common_TNN$DAYS_LAST_FOLLOWUP)

# remove any of the letters "a", "b" or "c", but only if they are at the end
# of the name, eg "stage iiia" would become simply "stage iii"
data_clinical_common_TNN$AJCC_PATHOLOGIC_TUMOR_STAGE = gsub("[ABC]$", "", data_clinical_common_TNN$AJCC_PATHOLOGIC_TUMOR_STAGE)

# we remove those with stage "not reported", since they are unknown
data_clinical_common_TNN[which(data_clinical_common_TNN$AJCC_PATHOLOGIC_TUMOR_STAGE == ""), "AJCC_PATHOLOGIC_TUMOR_STAGE"] = NA

table(data_clinical_common_TNN$AJCC_PATHOLOGIC_TUMOR_STAGE)

fit = survfit(Surv(overall_survival, deceased) ~ AJCC_PATHOLOGIC_TUMOR_STAGE, data=data_clinical_common_TNN)
...
sum(res_de$padj < 0.06, na.rm=TRUE)
