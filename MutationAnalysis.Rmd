...
mutation_TTN_patientID = data_mutation_common$PATIENT_ID[data_mutation_common$Hugo_Symbol == "TTN"]

data_clinical_common_TTN = subset(data_clinical_common, PATIENT_ID %in% mutation_TTN_patientID)
...
```{r}
Surv(data_clinical_common_TTN$overall_survival, data_clinical_common_TTN$deceased) ~ data_clinical_common_TTN$SEX

fit = survfit(Surv(overall_survival, deceased) ~ SEX, data = data_clinical_common_TTN)

table(data_clinical_common_TTN$SEX)
pval = surv_pvalue(fit, data=data_clinical_common_TTN)$pval
print(pval)

ggsurvplot(fit, data=data_clinical_common_TTN, pval=T, risk.table=T, risk.table.col="strata", risk.table.height=0.35)
```
SA on the most mutated gene tumor stage
```{r}
data_clinical_common_TTN$deceased = data_clinical_common_TTN$PFS_STATUS == "1:PROGRESSION"

# create an "overall survival" variable that is equal to days_to_death
# for dead patients, and to days_to_last_follow_up for patients who
# are still alive
data_clinical_common_TTN$overall_survival = ifelse(data_clinical_common_TTN$deceased,
                                   data_clinical_common_TTN$OS_MONTHS,
                                   data_clinical_common_TTN$DAYS_LAST_FOLLOWUP)

# remove any of the letters "a", "b" or "c", but only if they are at the end
# of the name, eg "stage iiia" would become simply "stage iii"
data_clinical_common_TTN$AJCC_PATHOLOGIC_TUMOR_STAGE = gsub("[ABC]$", "", data_clinical_common_TTN$AJCC_PATHOLOGIC_TUMOR_STAGE)


table(data_clinical_common_TTN$AJCC_PATHOLOGIC_TUMOR_STAGE)

fit = survfit(Surv(overall_survival, deceased) ~ AJCC_PATHOLOGIC_TUMOR_STAGE, data=data_clinical_common_TTN)
...
make data as factors so it runs faster (?)
```{r}
# data_rna_common_cluster = as.factor(data_rna_common_cluster)
```
