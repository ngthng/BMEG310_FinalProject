...
genes.ordered.top = genes.ordered[1:7]
...
### survival analysis 
```{r}
data_clinical_common$deceased = data_clinical_common$OS_STATUS == "1:DECEASED"
...
```{r}
ggsurvplot(fit, data=data_clinical_common, pval=T, risk.table=T, risk.table.height=0.35)
```
### gene expression
```{r}
data_rna_common <- data_rna_common[rowSums(data_rna_common)>1,]
metadata <- data.frame(
  patientID = common_names2,
  condition = data_clinical_common$SEX
)

dds <- DESeqDataSetFromMatrix(countData = data_rna_common, colData = metadata, design = ~ condition)
dds = DESeq(dds)
dds
res1 <- results(dds)
res1
