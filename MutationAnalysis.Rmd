...
genes.ordered.top20 = genes.ordered[1:20]
genes.ordered.top20.names = names(genes.ordered.top20)

data_oncoplot.top20 = subset(data_oncoplot, Hugo_Symbol %in% genes.ordered.top20.names)
...
```{r}
cnv_events = unique(data_oncoplot.top20$Variant_Classification)
oncomat.top20 = reshape2::dcast(
  data = data_oncoplot.top20,
  formula = Hugo_Symbol ~ Tumor_Sample_Barcode,
  fun.aggregate = function(x, cnv = cnv_events) {
    x = as.character(x) # >= 2 same/distinct variant classification = Multi_Hit
    xad = x[x %in% cnv]
...
hugo <- as.data.frame(table(data_oncoplot.top20$Hugo_Symbol))

rownames(oncomat.top20) = oncomat.top20$Hugo_Symbol
oncomat.top20 <- oncomat.top20[,-1]
oncomat.top20.ordered <- oncomat.top20[order(-hugo$Freq),]



```

### Transforming the top 20 matrix into binary

```{r, echo = FALSE}
mat.top20 <- oncomat.top20.ordered
mat.top20[mat.top20== "Silent"] = 0
mat.top20[mat.top20 == "Intron"] = 0
mat.top20[mat.top20 == "Missense_Mutation"] = 0

mat.top20[mat.top20 == ""] = 0


mat.top20 <- apply(mat.top20, 2 ,as.numeric)
mat.top20 <- as.matrix(mat.top20)
mat.top20[is.na(mat.top20)]=1

rownames(mat.top20)  <-  row.names(oncomat.top20.ordered)
```


```{r}
reduce.mat <- mat.top20
...
Get all the genes with 1, then pick out the top 20 occuring genes
